...
class EnhancedDocumentGenerator: 
    ...
    def _add_introduction(self, doc, application_info):
        ...
        description = application_info.get('description', 'This document describes the high-level design of the system.')
        doc.add_paragraph(description)

    def _add_data_architecture(self, doc, data_architecture_info):
        """Add data architecture section to the document"""
        doc.add_heading('Data Architecture', level=1)

        overview = data_architecture_info.get('overview', 'This section outlines the data architecture strategy including storage, modeling, and flow.')
        doc.add_paragraph(overview)

        # Add Data Stores
        data_stores = data_architecture_info.get('data_stores', [])
        if data_stores:
            doc.add_heading('Data Stores', level=2)
            for store in data_stores:
                name = store.get('name', 'Unnamed Store')
                store_type = store.get('type', 'Unknown Type')
                purpose = store.get('purpose', '')
                doc.add_paragraph(f"• {name} ({store_type}): {purpose}")

        # Add Data Models
        data_models = data_architecture_info.get('data_models', [])
        if data_models:
            doc.add_heading('Data Models', level=2)
            for model in data_models:
                name = model.get('name', 'Unnamed Model')
                attributes = ', '.join(model.get('attributes', []))
                doc.add_paragraph(f"• {name}: {attributes}")

        # Add Data Flows
        data_flows = data_architecture_info.get('data_flows', [])
        if data_flows:
            doc.add_heading('Data Flows', level=2)
            for flow in data_flows:
                source = flow.get('source', 'Unknown Source')
                destination = flow.get('destination', 'Unknown Destination')
                description = flow.get('description', '')
                doc.add_paragraph(f"{source} → {destination}: {description}")

        # Add Data Management
        data_mgmt = data_architecture_info.get('data_management', {})
        if data_mgmt:
            doc.add_heading('Data Management', level=2)
            for key, val in data_mgmt.items():
    ...
    def _add_system_overview(self, doc, application_info):
        ...
        overview = application_info.get('system_overview', 'The system is designed to meet business and technical requirements with modular components and scalable architecture.')
        doc.add_paragraph(overview)
    ...
    def _add_architecture_overview(self, doc, architecture_info):
        ...
        overview = architecture_info.get('overview', 'This section provides a high-level overview of the system architecture.')
        doc.add_paragraph(overview)

    def _add_architecture_principles(self, doc, architecture_info):
        """Add architecture principles section"""
        doc.add_heading('Architecture Principles', level=1)
    ...
    def _add_technology_stack(self, doc, tech_stack_info):
        ...
        overview = tech_stack_info.get('overview', 'This section describes the technologies used across different layers of the application.')
        doc.add_paragraph(overview)
        
        for category in tech_stack_info.get('categories', []):
            doc.add_heading(category.get('name', 'Technology Category'), level=2)
            for tech in category.get('technologies', []):
                ...
                tech_text = f"{name} (v{version}): {purpose}\nRationale: {rationale}"
                doc.add_paragraph(tech_text)

    def _add_security_architecture(self, doc, security_info):
        """Add security architecture section"""
        doc.add_heading('Security Architecture', level=1)
        overview = security_info.get('overview', 'This section describes the security mechanisms employed to protect the system and its data.')
        doc.add_paragraph(overview)
    ...
    def _add_deployment_architecture(self, doc, deployment_info):
        """Add deployment architecture section"""
        doc.add_heading('Deployment Architecture', level=1)
        overview = deployment_info.get('overview', 'The application is deployed across multiple environments to ensure robustness and reliability.')
        doc.add_paragraph(overview)
        ...
        if environments:
            ...
            for env in environments:
                doc.add_paragraph(f"{env.get('name', 'Unknown')}: {env.get('purpose', '')}")
        
        for key, val in deployment_info.items():
            if key in ['overview', 'environments']:
                continue
            doc.add_heading(key.replace('_', ' ').title(), level=2)
    ...
    def _add_operational_considerations(self, doc, operations_info):
        """Add operational considerations section"""
        doc.add_heading('Operational Considerations', level=1)
        overview = operations_info.get('overview', 'This section outlines monitoring, logging, backup, and DR strategies.')
        doc.add_paragraph(overview)
        
        for key, val in operations_info.items():
            if key == 'overview':
                continue
            doc.add_heading(key.replace('_', ' ').title(), level=2)
            if isinstance(val, dict):
                for subkey, subval in val.items():
                    doc.add_paragraph(f"{subkey.title()}: {subval}")
            else:
                doc.add_paragraph(str(val))
    ...
    def _add_appendices(self, doc, application_info):
        ...
        doc.add_paragraph('This section includes supporting information such as definitions and acronyms.')
        ...
        for term_def in application_info.get('definitions', [
            {'term': 'API', 'definition': 'Application Programming Interface'},
            {'term': 'HLDD', 'definition': 'High Level Design Document'}
        ]):
            row_cells = table.add_row().cells
            row_cells[0].text = term_def.get('term', '')
            row_cells[1].text = term_def.get('definition', '')
    ...
    def _load_patterns_knowledge_base(self):
        ...
        return {
            ...
            "event_driven": {
                ...
                "common_services": ["AWS Lambda", "SNS", "SQS", "EventBridge"],
                "strengths": ["scalability", "decoupling", "real-time processing"]
            },
            'layered': {
            ...
            "monolith": {
                "description": "A single unified application.",
                "common_services": ["Java EE", "Flask monolith", "Spring Boot"],
                "strengths": ["simplicity", "performance", "easier to develop early on"]
            }
        }
    ...
    def _load_technologies_knowledge_base(self):
        ...
        return {
            ...
            "AWS": {
                ...
                "integration": ["API Gateway", "SNS", "SQS"],
            ...
            "Google Cloud": {
                ...
                "integration": ["Pub/Sub", "API Gateway", "Cloud Tasks"],
                "monitoring": ["Cloud Monitoring", "Cloud Trace"]
            },
            "Oracle": {
                "compute": ["Compute Instances", "Functions", "Container Engine"],
                "database": ["Autonomous DB", "MySQL", "Oracle DB"],
                "storage": ["Object Storage", "Block Volume"],
                "network": ["VCN", "Load Balancer", "DNS"],
                "integration": ["API Gateway", "Streaming", "Integration Cloud"],
    ...
    def _generate_data_architecture(self, description, arch_style, tech_stack):
            ...
            def _add_introduction(self, doc, application_info):
                """Add introduction section with placeholder implementation"""
                doc.add_heading('Introduction', level=1)
                doc.add_paragraph(application_info.get('description', 'No description provided'))

            def _add_system_overview(self, doc, application_info):
                """Add system overview section with placeholder implementation"""
                doc.add_heading('System Overview', level=1)
                doc.add_paragraph('System overview details will be added here.')

            def _add_architecture_overview(self, doc, architecture_info):
                """Add architecture overview section with placeholder implementation"""
                doc.add_heading('Architecture Overview', level=1)
                doc.add_paragraph('Architecture overview details will be added here.')

            def _add_architecture_principles(self, doc, architecture_info):
                """Add architecture principles section with placeholder implementation"""
                doc.add_heading('Architecture Principles', level=1)
                doc.add_paragraph('Architecture principles will be added here.')

            def _add_technology_stack(self, doc, tech_stack_info):
                """Add technology stack section with placeholder implementation"""
                doc.add_heading('Technology Stack', level=1)
                doc.add_paragraph('Technology stack details will be added here.')

            def _add_security_architecture(self, doc, security_info):
                """Add security architecture section with placeholder implementation"""
                doc.add_heading('Security Architecture', level=1)
                doc.add_paragraph('Security architecture details will be added here.')

            def _add_deployment_architecture(self, doc, deployment_info):
                """Add deployment architecture section with placeholder implementation"""
                doc.add_heading('Deployment Architecture', level=1)
                doc.add_paragraph('Deployment architecture details will be added here.')

            def _add_operational_considerations(self, doc, operations_info):
                """Add operational considerations section with placeholder implementation"""
                doc.add_heading('Operational Considerations', level=1)
                doc.add_paragraph('Operational considerations will be added here.')

            def _add_appendices(self, doc, application_info):
                """Add appendices section with placeholder implementation"""
                doc.add_heading('Appendices', level=1)
                doc.add_paragraph('Appendices will be added here.')
...
def _generate_enhanced_component_description(self, name, description, arch_style):
    ...
    elif 'auth' in lower_name or 'security' in lower_name:  # Added colon here
        return (
